...
public class StatisticServiceImpl implements StatisticService {
    ...
    public Double countAverageMarkForUser(String userName) {
        ...
        return studentRepository.findByUserName(userName).getStudentMarks()
                .stream()
    ...
    public Double countAverageMarkForClassroom(String classroomName) {
        ...
        byClassroom.stream().forEach(student -> markList.addAll(student.getStudentMarks()));
    ...
    public Double countAverageMarkForSchool() {
        ...
        studentRepositoryAll.forEach(student -> markList.addAll(student.getStudentMarks()));
        return markList
                .stream()
                .mapToInt(Mark::getMarkValue)
                .average()
    ...
    public Map<Double, TreeSet<Student>> sortStudentsByAverageAsc() {
        ...
        List<Student> studentRepositoryAll = studentRepository.findAll();
        studentRepositoryAll.removeIf(student -> student.getStudentMarks().isEmpty());

        studentRepositoryAll.forEach(student -> {
            double average = student.getStudentMarks().stream().mapToInt(Mark::getMarkValue).average().orElse(0);
    ...
    public Map<Mark, Integer> countQuantityOfEachMarkForSchool() {
        ...
        studentRepositoryAll.forEach(student -> student.getStudentMarks().forEach(mark -> {
    ...
    public Map<Mark, Set<Student>> getFromClassroomMarkStudentListMap(String classroomName) {
        ...
        studentsInClassroom.forEach(student -> student.getStudentMarks().forEach(mark -> {
    ...
    public Map<Mark, Set<Student>> getFromSchoolMarkStudentListMap() {
        ...
        studentRepositoryAll
                .forEach(student -> student.getStudentMarks()
                        .forEach(mark -> {
                            if (!integerStudentMap.containsKey(mark)) {
                                Set<Student> students = new TreeSet<>();
                                students.add(student);
